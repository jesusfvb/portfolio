import { SKILLS_ICONS } from "@/domain/constants";
    ...
    <section id="skills" className="py-24 bg-[#1a1a1a]">
      <div className="container">
        ...
        <div className="flex flex-wrap gap-4 justify-center mt-12 max-w-[900px] mx-auto px-4 md:px-0">
          {Object.entries(SKILLS_ICONS).map(([skill, icon], index) => (
            <div 
              key={index} 
            ...
            >
              <span className="flex items-center justify-center text-xl">{icon.icon}</span>
              <span>{skill}</span>
            </div>
          ))}
        </div>
